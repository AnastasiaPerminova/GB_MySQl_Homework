...
CREATE OR REPLACE VIEW user_id1_friends
AS SELECT 
...
WHERE  f.target_user_id = 1 AND f.`status` = 'approved'
UNION 
 SELECT 
 ...
 WHERE  f.initiator_user_id = 1 AND f.`status` = 'approved';
 
 SELECT * FROM user_id1_friends;
 
-- 2. Создайте представление, в котором будут выводится все сообщения, в которых принимал  участие пользователь с id = 1.
CREATE OR REPLACE VIEW user_id1_messages 
AS SELECT 
	id,
	from_user_id,
    to_user_id,
...
CREATE OR REPLACE VIEW users_under20
AS SELECT
	CONCAT(u.firstname, " ", u.lastname) AS fullname,
    p.gender,
    p.hometown
...
WHERE (SELECT TIMESTAMPDIFF(YEAR, p.birthday, curdate())) <= 20;

SELECT * FROM users_under20;
    
-- 6.Найдите кол-во, отправленных сообщений каждым пользователем и выведите ранжированный список пользователей, указав имя и фамилию пользователя,
...
SELECT 
	tm.fullname,
	tm.total_messages,
	DENSE_RANK() OVER(ORDER BY tm.total_messages DESC) AS `dense_rank`
FROM (SELECT
		CONCAT(u.firstname, " ", u.lastname) AS fullname,
...
SELECT
    ...
    body,
    created_at,
    TIMESTAMPDIFF(DAY, created_at, LEAD(created_at) OVER(ORDER BY created_at)) as days_to_next_message
FROM messages;