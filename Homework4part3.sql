...
SELECT 
	(SELECT TIMESTAMPDIFF(YEAR, p.birthday, curdate()))as age,
     m.filename,
     COUNT(*) AS total_likes
FROM media m 
JOIN `profiles` p ON p.user_id = m.user_id
JOIN likes l ON l.media_id = m.id
WHERE (SELECT TIMESTAMPDIFF(YEAR, p.birthday, curdate())) < 12
GROUP BY media_id;
...
JOIN likes l ON l.media_id = m.id
WHERE (SELECT TIMESTAMPDIFF(YEAR, p.birthday, curdate())) < 12
GROUP BY (SELECT TIMESTAMPDIFF(YEAR, p.birthday, curdate()));
...
UNION 
 ...
 WHERE  f.initiator_user_id = 1 AND f.`status` = 'approved')  friends 
 LEFT JOIN messages m on friends.user_id = m.from_user_id
 WHERE to_user_id = 1
 GROUP by friends.fullname
