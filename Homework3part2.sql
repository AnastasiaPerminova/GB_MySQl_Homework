...
-- 1. Посчитать количество документов у каждого пользователя
SELECT
...
-- 2.- Посчитать лайки для моих документов (моих медиа)
SELECT
	users.firstname,
	COUNT(likes.id) AS count_likes
FROM likes, users, media
WHERE  likes.media_id = media.id  AND media.user_id = users.id AND users.id = 1;
